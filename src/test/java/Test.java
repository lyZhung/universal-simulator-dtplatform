...
public class Test {
    ...
    public void testJsonArrayByDocument4(){
        String json = JsonUtils.getJson("F:\\source\\universal-simulator-dtplatform\\src\\main\\resources\\test5.json");
        BsonArray bsonArray1 = BsonArray.parse(json);
        BsonArray bsonArray2 = new BsonArray();
        List<BsonDocument> bsonList = new ArrayList<BsonDocument>();
        for (BsonValue bsonValue : bsonArray1) {
            bsonArray2.add(bsonValue.asDocument());
        }

        System.out.println(bsonArray2.toString());
        String bson = bsonArray2.toString();
        String substring = bson.substring(bson.indexOf("["), bson.lastIndexOf("]") + 1);
